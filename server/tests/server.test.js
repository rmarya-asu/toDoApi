...
describe('GET /todos/:id', () => {
    ...
    it('should not return the todo doc created by another user', (done) => {
        request(app)
            .get(`/todos/${todoData[1]._id.toHexString()}`)
            .set('x-auth', usersData[0].tokens[0].token)
            .expect(404)
            .end(done);
    });
...
describe('DELETE /todos/:id', () => {
    ...
    it('user 2 should not be able remove a todo from user1', (done) => {
        var hexId = todoData[0]._id.toHexString();
        request(app)
            .delete(`/todos/${hexId}`)
            .set('x-auth', usersData[1].tokens[0].token)
            .expect(404)
...
describe('PATCH /todos/:id', () => {
    ...
    it('should not update a todo if user is not authenticated', (done) => {
        var text = "test update todo";
        var hexId = todoData[0]._id.toHexString();
        request(app)
            .patch(`/todos/${hexId}`)
    ...
    it('should login user and return auth token', (done) => {
        request(app)
            ...
            .end((err, res) => {
                ...
                User.findById(usersData[2]._id).then((user) => {
...
describe('DELETE /users/me/token', () => {
    it('should remove auth token on logout', (done) => {
        request(app)
            ...
            .expect(200)
            .end((err, res) => {
                User.findById(usersData[0]._id).then((user) => {
                    expect(user.tokens.length).toBe(0);
                    done();
                }).catch((e) => done(e));
            })
